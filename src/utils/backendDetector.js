...
class BackendDetector {
  ...
  static detectFramework(dirPath, packageJson) {
    ...
    // Generic Node.js
    if (packageJson && !this.hasDependency(packageJson, "react") && 
        !this.hasDependency(packageJson, "vue") && !this.hasDependency(packageJson, "svelte")) {
  ...
  static detectArchitecture(dirPath) {
    ...
    for (const [arch, folders] of Object.entries(patterns)) {
      const matchCount = folders.filter(folder => 
  ...
  static findSrcFolder(dirPath) {
    ...
    return dirPath; // Use root if no src folder found
  }
  ...
  static getBackendBaseFolder(projectInfo) {
    // For monorepo or microservices, might want to create in specific location
    ...
    // For standard projects, use src folder if exists
    return projectInfo.srcFolder || projectInfo.basePath;
  }
}

module.exports = { BackendDetector };